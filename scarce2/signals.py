...
def collected_charge_vs_time_fast(
    sensor: Sensor, initial_pos: Tuple[float, float] = (0, 0), timestep: float = 0.1
) -> dict:
    """Propagate an electron hole pair through the sensor and calculate the charge signal induced by each type of charge carrier.
    ...
    charge_e = _calc_charge_vs_time_fast(initial_pos, pot, timestep, sensor.n_pixel, sensor.pitch, sensor.thickness, 
            current_pos['electrons'], mobility['electrons'], e_field_x, e_field_y)
    

    charge_h = _calc_charge_vs_time_fast(initial_pos, pot, timestep, sensor.n_pixel, sensor.pitch, sensor.thickness, 
            current_pos['holes'], mobility['holes'], e_field_x, e_field_y, conv_factor=-1)
    return charge_e, charge_h

@njit
def _calc_charge_vs_time_fast(initial_pos, pot, timestep, n_pixel, pitch, thickness, current_pos, mobility, e_field_x, e_field_y, conv_factor=1):
    current_wf = _calculate_field(initial_pos, pot, (n_pixel * pitch, thickness))
    charge = []
    while True:
        e_field_at_pos = (
            _calculate_field(current_pos, e_field_x, (n_pixel * pitch, thickness)),
            _calculate_field(current_pos, e_field_y, (n_pixel * pitch, thickness)),
        )
        current_pos = _propagate_charge(
            current_pos, e_field_at_pos, mobility, timestep
        )

        stop_cond_pos =  current_pos[1] >= thickness - 0.0025 or current_pos[1] <= 0.0025

        stop_cond_pos |= (
            abs(current_pos[0] - (n_pixel * pitch)) <= 0.0025
        )  # charge reached side of sensor

        new_wf = _calculate_field(current_pos, pot, (n_pixel * pitch, thickness))
        charge_per_step = 0.001 * (new_wf - current_wf)  # scale down to proper value after propagation

        if stop_cond_pos:
            break

        charge.append(conv_factor*charge_per_step)
        current_wf = new_wf
    return charge
